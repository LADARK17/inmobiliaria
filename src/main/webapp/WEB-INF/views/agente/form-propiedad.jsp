    ...
    fotosGaleria.forEach((url, idx) => {
        ...
        col.innerHTML = `
            <div class="card h-100 border shadow-sm rounded-3 overflow-hidden position-relative">
                <img src="${url}" class="card-img-top object-fit-cover" style="height: 140px;" 
                     onerror="this.src='https://images.unsplash.com/photo-1560518883-ce09059eeffa?w=800'">
                
                <span class="badge ${esPrincipal ? 'bg-primary' : 'bg-dark bg-opacity-75'} position-absolute top-0 start-0 m-2 small">
                    ${esPrincipal ? '<i class="bi bi-star-fill text-warning me-1"></i>Principal' : '#' + (idx + 1)}
                </span>

                <div class="card-body p-2 d-flex justify-content-between align-items-center bg-white border-top">
                    ${!esPrincipal ? `
                        <button type="button" class="btn btn-outline-primary btn-sm py-0 px-2" title="Establecer como foto principal" onclick="establecerComoPrincipal(${idx})">
                            <i class="bi bi-star"></i> Principal
                        </button>
                    ` : '<span class="small fw-bold text-primary">Portada</span>'}
                    
                    <button type="button" class="btn btn-outline-danger btn-sm py-0 px-2" title="Eliminar fotografía" onclick="eliminarFoto(${idx})">
